# glossary_term.py
# "개미 용어 사전" 페이지 전용 테이블. timeline 도메인의 glossary.py(하드코딩 dict, 메인 페이지
# "오늘의 개미 용어 한 입"이 쓰는 그 파일)와는 완전히 분리된 별도 테이블이다 - 서로 데이터를
# 공유하지 않는다. glossary.py는 이번 작업에서 건드리지 않는다.
#
